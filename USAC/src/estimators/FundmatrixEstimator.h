...
double  calcFundSampsonErr(double m1[3],double m2[3],double f[9])
{
  const double rxc = f[0] * (m2[0]) + (f[3]) * (m2[1]) + (f[6]);
  const double ryc = (f[1]) * (m2[0]) + (f[4]) * (m2[1]) + (f[7]);
  const double rwc = (f[2]) * (m2[0]) + (f[5]) * (m2[1]) + (f[8]);
  const double r =((m1[0]) * rxc + (m1[1]) * ryc + rwc);
  const double rx = f[0] * (m1[0]) + (f[1]) * (m1[1]) + (f[2]);
  const double ry = (f[3]) * (m1[0]) + (f[4]) * (m1[1]) + (f[5]);
  return  r*r / (rxc*rxc + ryc*ryc + rx*rx + ry*ry);
}

double  calcFundSymmetricEpipolarErr(double m1[3],double m2[3],double f[9])
{
  const double rxc = f[0] * (m2[0]) + (f[3]) * (m2[1]) + (f[6]);
  const double ryc = (f[1]) * (m2[0]) + (f[4]) * (m2[1]) + (f[7]);
  const double rwc = (f[2]) * (m2[0]) + (f[5]) * (m2[1]) + (f[8]);
  ...
  for (unsigned int i = 0; i < usac_num_data_points_; ++i)
    {
      ...
      if (usac_error_function_ == USACConfig::ERR_SYMMETRIC_TRANSFER_MAX) {
          std::cerr << "ERR_SYMMETRIC_TRANSFER_MAX not implemented. Falling back to ERR_SYMMETRIC_TRANSFER_SUM" << std::endl;
        }
      if (usac_error_function_ == USACConfig::ERR_SYMMETRIC_TRANSFER_SUM) {
          pt = input_points_denorm_ + 6*pt_index;
          double m1[3];
          m1[0] = pt[0]; m1[1] = pt[1]; m1[2] = pt[2];
          double m2[3];
          m2[0] = pt[3]; m2[1] = pt[4]; m2[2] = pt[5];

          temp_err  = calcFundSymmetricEpipolarErr(m1,m2,model);
          ...
          temp_err  = calcFundSampsonErr(m1,m2,model);
        }
