...
protected:
  ...
  bool usac_check_sampson_with_symmetric;
...
protected:
  ...
  bool validModelBySymCheck(unsigned int modelIdx) {
    //storing parameters in order to release them
    const double current_threshold = usac_inlier_threshold_;
    const USACConfig::GeometricErrorFunction curr_err_funct =  usac_error_function_;
    std::swap( err_ptr_[0], err_ptr_[2]) ; //Save model

    usac_error_function_ = USACConfig::ERR_SYMMETRIC_TRANSFER_SUM;
    usac_inlier_threshold_*= k_usac_CheckCoef;
    unsigned int inlier_count_test, num_points_tested_test;

    bool good_model1 =  static_cast<ProblemType *>(this)->evaluateModel(modelIdx, &inlier_count_test, &num_points_tested_test);
    bool degenerate_model_by_sym_check = (inlier_count_test < (usac_min_sample_size_ + 3));
    //    std::cerr << "inlier_count_test=" << inlier_count_test << std::endl;
    if (degenerate_model_by_sym_check) {
        usac_results_.rejected_by_sym_check_count++;
      }
    //restore parameters
    usac_error_function_ = curr_err_funct;
    usac_inlier_threshold_ = current_threshold;
    std::swap( err_ptr_[0], err_ptr_[2]) ; //Restore model
    return !degenerate_model_by_sym_check;
  }
};
...
void USAC<ProblemType>::initParamsUSAC(const ConfigParams& cfg)
{
  ...
  usac_check_sampson_with_symmetric = cfg.common.check_with_Symmetric;
...
void USAC<ProblemType>::initDataUSAC(const ConfigParams& cfg)
{
  ...
  errs_.clear(); errs_.resize(3*usac_num_data_points_, 0);
  err_ptr_[0] = errs_.begin();
  err_ptr_[1] = errs_.begin()+usac_num_data_points_;
  err_ptr_[2] = errs_.begin()+2*usac_num_data_points_;
...
bool USAC<ProblemType>::solve()
{
  ...
  while (usac_results_.hyp_count_ < adaptive_stopping_count && usac_results_.hyp_count_ < usac_max_hypotheses_)
    {
            ...
            case USACConfig::VERIF_STANDARD:
              {
                ...
                if (inlier_count > usac_results_.best_inlier_count_)
                  {
                    if (usac_check_sampson_with_symmetric) {
                        if (!validModelBySymCheck(i))
                          continue;
                      }
                    update_best = true;
                    ...
                    if (inlier_count > usac_results_.best_inlier_count_)
                      {
                        ...
                        std::cerr << "new " << inlier_count << " old: " <<   usac_results_.best_inlier_count_ << std::endl;
                        update_best = true;
                        ...
                        usac_results_.best_model_index = i;
                        designSPRTTest();
      ...
      if (update_best && usac_test_degeneracy_)
        {
          std::cerr << "Testing for degeneracy (" << usac_results_.best_inlier_count_ << ")" << std::endl;
      ...
      if (usac_local_opt_method_ == USACConfig::LO_LOSAC && update_best == true)
        {
          std::cerr << "(" << usac_results_.hyp_count_ << ") Performing LO. Inlier count before: " << usac_results_.best_inlier_count_;
          ...
          std::cerr << ". After: " << usac_results_.best_inlier_count_ << ", lo-inlier count: " << lo_inlier_count << std::endl;
  ...
  std::cerr << "Number of models rejected by transfer error check: " << usac_results_.rejected_by_sym_check_count << std::endl;
  std::cerr << "Number of verifications per model: " <<
               (double)usac_results_.total_points_verified_/(usac_results_.model_count_-usac_results_.rejected_model_count_) << std::endl;
  std::cerr << "Max inliers/total points: " << usac_results_.best_inlier_count_ << "/" << usac_num_data_points_ << std::endl;

  //  for (int ttt=0; ttt < usac_num_data_points_; ttt++) {
  //      std::cerr << "inl=" << usac_results_.inlier_flags_[ttt] << std::endl;
  //    }
...
//    }
  ...
  std::cerr << " (" << num_elements_intersection << ") " ;
      ...
      if (temp_inliers > lo_inliers)
        {
          ...
          if (usac_check_sampson_with_symmetric) {
              const double current_threshold = usac_inlier_threshold_;
              usac_inlier_threshold_ *= lo_threshold_multiplier_;
              if (!validModelBySymCheck(0))
                {
                  usac_inlier_threshold_ = current_threshold; //restore
                  continue;
                }
              usac_inlier_threshold_ = current_threshold;  //restore
...
void USAC<ProblemType>::storeSolution(unsigned int modelIndex, unsigned int numInliers)
{
  ...
  if (num_inl != numInliers) {
      std::cerr << "something weird. num_inl= " << num_inl << " " << numInliers << std::endl;
    }

  usac_results_.best_inlier_count_ = num_inl;
